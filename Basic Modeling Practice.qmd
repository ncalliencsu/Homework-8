...
bd_sub <- bike_data |> 
  ...
  summarise(across(where(is.numeric) , mean, na.rm = TRUE, .names = '{col}_MEAN'),
            ...
            select(DATE, SEASONS, HOLIDAY, starts_with("SUM"), everything(), -RBC_MEAN, -HOUR_MEAN)
...
corr <- bd_sub |> 
  ungroup() |> 
  select(where(is.numeric), -`RAIN_FALL(mm)_MEAN`, -`SNOW_FALL(cm)_MEAN`) |> 
  cor()
...
The Correlation Table indicates a positive correlation between Daily Bike Rentals and Temperature, Dew Point and Temperature. There is a strong positive correlation between Temperature and Dew Point Temperature. There seems to be a moderate postive correlation between Dew Point Temperature and Relative Humidity.
...
data_split <- initial_split(bd_sub, prop = 3/4, strata = SEASONS)
               
# Create data frames for the two sets:
bike_train_data <- training(data_split)
bike_test_data  <- testing(data_split)
...
recipe2 <- 
 ...
 step_interact(
  terms = ~ starts_with("HOLIDAY"):starts_with("SEASONS") +
           ...
           `TEMP(deg C)_MEAN`:`RAIN_FALL(mm)_MEAN`
)
...
Consider the training set CV error to choose a best model.  
```{r}
...
predictions <- predict(final_model, bike_test_data)
RMSE(predictions, bike_test_data$SUM_RBC)
